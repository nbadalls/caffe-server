...
template <typename Dtype>
int TripletImageDataLayer<Dtype>::MyRandomData (int i) {
...
void TripletImageDataLayer<Dtype>::ShuffleImages() {
  ...
  std::random_shuffle(tnum.begin(), tnum.end(), TripletImageDataLayer<Dtype>::MyRandomData);
  tlines_.clear();
  for(int i = 0; i < num_images / pairsize; i ++)
  {
