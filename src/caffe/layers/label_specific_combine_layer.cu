...
namespace caffe {
    ...
    CUDA_KERNEL_LOOP(i, n) {
      ...
      Dtype tmp = cos_theta;
       ...
       if ((sq_cos_theta <= (Dtype)1.) && (sq_cos_theta >= (Dtype)-1.)){
           ...
           tmp = cos_theta*cos_margin_theta - sin_theta*sin_margin_theta + margin_m;
       }
  ...
  template <typename Dtype>
  void LabelSpecificCombineLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    LabelSpecificCosineSubForward<Dtype> << <CAFFE_GET_BLOCKS(num), CAFFE_CUDA_NUM_THREADS >> > (
      num, dim, label_data, bottom_data, top_data, cos_margin_theta, sin_margin_theta,margin_m);
  ...
  template <typename Dtype>
  void LabelSpecificCombineLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    ...
    if (top[0] != bottom[0] && propagate_down[0]) {
      ...
      LabelSpecificCosineSubBackward<Dtype> << <CAFFE_GET_BLOCKS(num), CAFFE_CUDA_NUM_THREADS >> > (
        num, dim, label_data, bottom_data, top_diff, bottom_diff, cos_margin_theta, sin_margin_theta);
      CUDA_POST_KERNEL_CHECK;
    }
  }

  INSTANTIATE_LAYER_GPU_FUNCS(LabelSpecificCombineLayer);
}  // namespace caffe
