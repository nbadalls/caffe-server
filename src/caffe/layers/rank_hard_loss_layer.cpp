...
void RankHardLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
	...
	CHECK_EQ(bottom.size(), 2) << "RankHardLossLayer Layer takes two blobs as input.";
...
void RankHardLossLayer<Dtype>::set_mask(const vector<Blob<Dtype>*>& bottom)
{
	...
	int neg_num = this->layer_param_.rank_hard_loss_param().neg_num();
	int pair_size = this->layer_param_.rank_hard_loss_param().pair_size();
	...
	int num = bottom[0]->num();  //batch num
