...
class ModelTest():
    ...
    def runTest(self, model_path):
            ...
            if len(select_model_list[0]) == 0:
                ...
                model_prefix = self.model_path.split('/')[-1] #folder name
                #get patch info
                ...
                loss_type = model_prefix.split('_')[0] #AMImageMeanCdata
                ...
                src_deploy_path = '{}/jobs/{}/{}/{}'.format(ConfigPath.local_caffe_path1, loss_type, model_prefix, deploy_file)
                if not os.path.exists(src_deploy_path):
                    src_deploy_path = '{}/jobs/{}/{}/{}'.format(ConfigPath.local_caffe_path2, loss_type, model_prefix, deploy_file)
                if not os.path.exists(src_deploy_path):
                    print('deploy file {} is missing'.format(src_deploy_path))
                    return 
                ...
                for index, elem in enumerate(select_model_list):
                    ...
                    f.write('cd {}\n'.format(ConfigPath.local_caffe_path1))
    ...
    def curvePrecious(self, output_result_path):
        statistic_info = []
        model_list = [elem for elem in os.listdir(output_result_path) if elem.find('result') >=0 and elem.endswith('.txt')] #find selected date's model
