...
import face_recognition_score_transform
...
def calculate_transform_score(info_dict):
	
	score_level1 = info_dict[1e-4]
	score_level2 = info_dict[1e-2]
	score_level3 = (score_level2 + score_level1)*1.3
	score_info = []
	for tolerance in [10**-7, 10**-6,10**-5, 10**-4, 10**-3, 10**-2, 10**-1]:
		distance = info_dict[tolerance]
		t_score = face_recognition_score_transform.score_transform(score_level1, score_level2, score_level3,distance)
		tmp_score_info = "threshold\t{}\tscore\t{}\tfpr\t{}".format(round(distance,2), round(t_score,2), tolerance)
...
def roc_maker(score_file, save_roc_info=False):
	...
	score_info = calculate_transform_score(info_dict)
